...
<template>
     <li class="flex w-full justify-between text-gray-700 cursor-pointer items-center mb-6">
        <a href="javascript:void(0)" 
          @click="showSubs"
         class="flex items-center focus:outline-none focus:ring-2 
         hover:bg-blue-400
         focus:ring-white w-full">
        <ChevronDownIcon/>
            <span class="text-sm ml-2">{{item.title}}</span>
        </a>
        <div v-if="item.badgeNumber" class="py-1 px-3   rounded   flex items-center justify-center text-xs">{{item.badgeNumber}}</div>
    </li>
